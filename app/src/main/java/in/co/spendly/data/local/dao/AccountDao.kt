...
interface AccountDao {
    ...
    /**
     * Get all accounts as a snapshot (one-shot query for export).
    ...
    /**
     * Delete all custom accounts.
     * Used during import to clear user-created accounts before restoring.
     * Predefined accounts (is_custom = 0) are preserved.
     *
     * Note: This should only be called after reassigning all transactions,
     * as accounts have FK RESTRICT.
     */
    @Query("DELETE FROM accounts WHERE is_custom = 1")
    suspend fun deleteCustomAccounts()
}
