 ...
 * When receipt deletion fails after database removal, orphaned files may accumulate.
 * Future implementation: Create a WorkManager periodic job to scan receipts directory
 * and remove files not referenced in the database. This cleanup job should:
 * 1. Query all receipt file paths from database
 * 2. Scan receipts directory for actual files
 * 3. Delete files not in the database query results
 * 4. Run periodically (e.g., weekly) when device is idle and charging
 */
@Singleton
    ...
    override suspend fun deleteReceipt(receipt: Receipt) {
        try {
            ...
            if (!FileUtils.deleteReceiptFile(context, receipt.filePath)) {
                Log.w(
                    "ReceiptRepository",
                    "Receipt deleted from DB but file removal failed: ${receipt.filePath}"
                )
        ...
        try {
            ...
            entity?.let {
                ...
                if (!FileUtils.deleteReceiptFile(context, it.filePath)) {
                    Log.w(
                        "ReceiptRepository",
                        "Receipt deleted from DB but file removal failed: ${it.filePath}"
                    )
                    // File deletion failed, but database record is already gone
