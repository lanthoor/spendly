...
class EnrichSmsTransactionsUseCaseTest {
    ...
    private class FakeExpenseRepository(
        ...
        override suspend fun getSmsLinkedExpensesSince(minSmsTimestamp: Long): List<Expense> = expenses
        override suspend fun getExpensesByIds(ids: List<Long>): List<Expense> = expenses.filter { it.id in ids }
    }
    ...
    private class FakeIncomeRepository(
        ...
        override fun getAllIncome(): Flow<List<Income>> = flowOf(incomes)
        ...
        override suspend fun getSmsLinkedIncomeSince(minSmsTimestamp: Long): List<Income> = incomes
        override suspend fun getIncomeByIds(ids: List<Long>): List<Income> = incomes.filter { it.id in ids }
    }
